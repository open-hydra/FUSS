...
SVG_OUT      = os.path.join(os.path.dirname(__file__), "2D-steady-state-qvol-plate-field.svg")
TARGET_ZONE  = "Block1"
TARGET_X     = 0.3
TARGET_Y     = 0.2
REFERENCE_T  = 273.15 + 310.1 # K
...
def save_colormap(x_cc, y_cc, T_2d, point_B, svg_path):
    ...
    # Boundary outline
    x0b = float(x_1d[0]  - (x_1d[1]  - x_1d[0])  / 2)
    x1b = float(x_1d[-1] + (x_1d[-1] - x_1d[-2]) / 2)
    y0b = float(y_1d[0]  - (y_1d[1]  - y_1d[0])  / 2)
    y1b = float(y_1d[-1] + (y_1d[-1] - y_1d[-2]) / 2)
    ax.plot([x0b, x1b, x1b, x0b, x0b],
            [y0b, y0b, y1b, y1b, y0b],
            "k-", linewidth=1.2)

    # Point B marker
    ax.plot(point_B[0], point_B[1], "o",
            color="cyan", markersize=7, markeredgecolor="black",
            markeredgewidth=0.8,
            label=f"P ({point_B[0]}, {point_B[1]}) m",
            zorder=5)
    ...
    ax.set_title(
        ...
        r"Volumetric heat generation $\dot{q} = 10^6$ W/m³",
        fontsize=11,
        fontweight="bold"
    )
    ax.set_aspect("equal")
...
def main():
    ...
    # Bilinear interpolation at point B
